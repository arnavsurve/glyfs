...
:::tip Identical Request Format
The request format is identical to the [Invoke API](/api/invoke). Only the response format differs.
:::
...
:::info Tool Event Variability
Tool event structure varies based on the MCP server and tool implementation. See [Tools & MCP](/tools/) documentation for specific tool event formats.
:::
...
For simple request/response scenarios, consider the [Invoke API](/api/invoke) instead.
...
:::info Token Counting
Token counts in the `done` event are approximated using `length / 4` and may not reflect actual LLM provider token usage.
:::
